...
import com.chatpurity.util.PermissionUtils;
...
public class ChatPurityCommand {
    ...
    /**
     * 验证字符串是否安全（防止 YAML 注入和其他安全问题）
     * @param input 要验证的字符串
     * @return 如果字符串安全返回 true，否则返回 false
     */
    private static boolean isYamlSafe(String input) {
        if (input == null || input.isEmpty()) {
            return false;
        }
        
        // 检查控制字符（不允许换行、回车等控制字符）
        for (char c : input.toCharArray()) {
            if (c < 0x20 && c != '\t') { // 允许制表符，禁止其他控制字符
                return false;
            }
        }
        ...
        // YAML 标签注入 (如 !!javax.script.ScriptEngineManager)
        if (input.contains("!!")) {
            return false;
        }
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        var root = literal("chatpurity")
            .requires(source -> PermissionUtils.hasPermissionLevel(source, OP_LEVEL));
    ...
    private static int addBlacklist(CommandContext<ServerCommandSource> context, String args) {
        ...
        if (!finalMode.isEmpty() && !isYamlSafe(finalMode)) {
