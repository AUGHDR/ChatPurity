...
public class ChatHandler {
    ...
    private int getPlayerOpLevel(ServerPlayerEntity player) {
        return PermissionUtils.getPermissionLevel(player);
    }
    ...
    private boolean isMuted(String playerName) {
        ...
        if (currentTime >= muteEndTime) {
            // 使用 computeIfPresent 原子地检查并移除，避免竞态条件
            mutedPlayers.computeIfPresent(playerName, (key, endTime) -> {
                // 再次检查时间，确保在并发情况下不会错误移除
